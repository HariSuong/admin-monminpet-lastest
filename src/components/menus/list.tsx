...
import ToggleCell from '@/components/menus/toggle-cell'
...
import { MenuListPag } from '@/schemaValidations/menus.schema'

interface MenuCrudGridProps {
  initialMenus: MenuListPag
  sessionToken: string
}
...
export default function MenuList({
  ...
  const [rows, setRows] = useState(initialMenus.data)
  const { showToast } = useToast()

  const [paginationModel, setPaginationModel] = useState({
    page: initialMenus.current_page - 1,
    pageSize: initialMenus.per_page
  })

  const [rowCount, setRowCount] = useState(initialMenus.total)
  ...
  const fetchData = useCallback(
    async (page: number, pageSize: number) => {
      try {
        setLoading(true)
        const { payload } = await menuApiRequest.getAllMenus(
          sessionToken,
  ...
  const handleDeleteClick = async (id: number) => {
    if (confirm('Bạn có chắc chắn muốn xóa chuyên mục này?')) {
      ...
      // Gọi API xóa:
      try {
        const result = await menuApiRequest.deleteMenu(id)
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: params => (
        <ToggleCell
          field='hot'
          menu={params.row} // Truyền toàn bộ dữ liệu chuyên mục
          onSuccess={() =>
            fetchData(paginationModel.page, paginationModel.pageSize)
          }
        />
  ...
  return (
    <Box sx={{ height: '100vh', width: '100%' }}>
      <DataGrid
        ...
        loading={loading}
        slots={{
          toolbar: () => <CustomToolbar href='/menus/add' />
        }}
        showToolbar
        localeText={{
